...
ESX.RegisterServerCallback('t1ger_deliveryjob:fetchShopOrders',function(source, cb)
    ...
    if Config.T1GER_Shops then
        local shop_orders = {}
        MySQL.Async.fetchAll("SELECT * FROM t1ger_orders", {}, function(results)
            if #results > 0 then
                for _,y in pairs(results) do
                    local orderData = nil
                    for k,v in pairs(json.decode(y.data)) do 
                        orderData = {shopID = v.shopID, itemLabel = v.itemLabel, orderItem = v.orderItem, curCount = v.curCount, orderQty = v.orderQty, itemPrice = v.itemPrice, pos = v.pos, orderCost = v.orderCost, shelf = v.shelf}
                    end
                    table.insert(shop_orders, {id = y.id, data = orderData, taken = y.taken})
                end
            end
            cb(shop_orders)
        end)
...
AddEventHandler('t1ger_deliveryJob:updateOrderState', function(orderData, state)
    MySQL.Async.fetchAll("SELECT * FROM t1ger_orders", {}, function(results)
        if #results > 0 then
            for k,v in pairs(results) do
                if orderData.id == v.id and orderData.data.shopID == v.shopID then
                    v.taken = 1
                    MySQL.Sync.execute("UPDATE t1ger_orders SET taken = @taken WHERE shopID = @shopID and id = @id", {
                        ['@taken'] = 1,
                        ['@shopID'] = orderData.data.shopID,
                        ['@id'] = orderData.id
                    })
                end
            end
...
AddEventHandler('t1ger_deliveryJob:orderDeliveryDone', function(orderData)
    MySQL.Async.fetchAll("SELECT * FROM t1ger_orders", {}, function(results)
        if #results > 0 then
            for k,v in pairs(results) do
                if orderData.id == v.id and orderData.data.shopID == v.shopID then
                    MySQL.Async.execute("DELETE FROM t1ger_orders WHERE id=@id", {['@id'] = orderData.id}) 
